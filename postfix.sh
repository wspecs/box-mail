...
edit_config /etc/postfix/main.cf \
  inet_interfaces=all \
  smtp_bind_address=$PRIVATE_IP \
  smtp_bind_address6=$PRIVATE_IPV6 \
  myhostname=$PRIMARY_HOSTNAME\
  smtpd_banner="\$myhostname ESMTP Hi, I'm a wspecsbox (Ubuntu/Postfix; see https://wspecsbox.email/)" \
...
edit_config /etc/postfix/master.cf -s -w \
...
cp postfix_outgoing_mail_header_filters /etc/postfix/outgoing_mail_header_filters
...
edit_config /etc/postfix/main.cf virtual_transport=lmtp:[127.0.0.1]:10025
# Because of a spampd bug, limit the number of recipients in each connection.
# See https://github.com/mail-in-a-box/wspecsbox/issues/1523.
edit_config /etc/postfix/main.cf lmtp_destination_recipient_limit=1
...
edit_config /etc/default/postgrey \
  POSTGREY_OPTS=\"'--inet=127.0.0.1:10023 --delay=180'\"


# We are going to setup a newer whitelist for postgrey, the version included in the distribution is old
cat > /etc/cron.daily/wspecsbox-postgrey-whitelist << EOF
#!/bin/bash
...
EOF
chmod +x /etc/cron.daily/wspecsbox-postgrey-whitelist
/etc/cron.daily/wspecsbox-postgrey-whitelist
