...
# but dovecot-lucene is packaged by *us* in the wspecsbox PPA,
# not by Ubuntu.
...
edit_config /etc/dovecot/conf.d/10-master.conf \
...
edit_config /etc/dovecot/conf.d/10-auth.conf \
...
edit_config /etc/dovecot/conf.d/10-ssl.conf \
  ssl=required \
...
# by a peer. See [#129](https://github.com/mail-in-a-box/wspecsbox/issues/129)
# and [How bad is IMAP IDLE](http://razor.occams.info/blog/2014/08/09/how-bad-is-imap-idle/).
edit_config /etc/dovecot/conf.d/20-imap.conf \
...
edit_config /etc/dovecot/conf.d/20-pop3.conf \
...
edit_config /etc/dovecot/conf.d/15-lda.conf \
...
# permission later.
cp sieve-spam.txt /etc/dovecot/sieve-spam.sieve
